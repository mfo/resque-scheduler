...
context 'scheduling jobs with hooks' do
  ...
  test 'before_schedule hook that does not return false should be enqueued' do
    ...
    SomeRealClassWithScheduleHooks.expects(:before_schedule_example).with("foo")
    SomeRealClassWithScheduleHooks.expects(:after_schedule_example).with("foo")
    Resque.enqueue_at(enqueue_time.to_i, SomeRealClassWithScheduleHooks, "foo")
  ...
  test 'before_schedule hook that returns false should not be enqueued' do
    enqueue_time = Time.now
    SomeRealClassWithScheduleHooks.expects(:before_schedule_example).with(:foo).returns(false)
    SomeRealClassWithScheduleHooks.expects(:after_schedule_example).never
    Resque.enqueue_at(enqueue_time.to_i, SomeRealClassWithScheduleHooks, :foo)
  ...
  test 'default failure hooks are called when enqueueing a job fails' do
    config = {
      'cron' => '* * * * *',
      'class' => 'SomeRealClassWithScheduleHooks',
      'args' => '/tmp'
    }
